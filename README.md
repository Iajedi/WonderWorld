...
For the installation to be done correctly, please proceed only with CUDA-compatible GPU available. It requires 48GB GPU memory to run (29 GB with offloading).
...
```sh
cd backbone && pip install -e ".[torch]" && cd ..
```
...
We utilise InternVL 3.5 in the report, but you can also use Gemini as the LLM with an API key:
...
- Run
  ...
  On your local laptop, `cd stream-viewer` and install `npm` requirements: `npm i`.
  ...
  In the `stream-viewer` directory, execute `npm run dev` on your local machine, and you should see the scene in it. You can navigate with `WSAD` and arrow keys.
