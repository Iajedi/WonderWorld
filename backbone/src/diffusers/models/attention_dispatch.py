...
if torch.__version__ >= "2.4.0":
    ...
    def _custom_op(name, fn=None, /, *, mutates_args, device_types=None, schema=None):
        # Some torch builds can fail schema inference when annotations are postponed strings.
        # If registration fails, continue with a no-op decorator so import does not crash.
        decorator = torch.library.custom_op(name, None, mutates_args=mutates_args, device_types=device_types, schema=schema)

        def wrap(func):
            try:
                return decorator(func)
            except Exception as e:
                logger.warning(f"Skipping custom_op registration for '{name}': {e}")
                return func

        return wrap if fn is None else wrap(fn)

    def _register_fake(op, fn=None, /, *, lib=None, _stacklevel=1):
        decorator = torch.library.register_fake(op, None, lib=lib, _stacklevel=_stacklevel)

        def wrap(func):
            try:
                return decorator(func)
            except Exception as e:
                logger.warning(f"Skipping register_fake for '{op}': {e}")
                return func
...
def _native_attention_forward_op(
    ...
    if enable_gqa and not _SDPA_SUPPORTS_ENABLE_GQA:
        raise ValueError("`enable_gqa` is not supported by this PyTorch scaled_dot_product_attention build.")
    sdpa_kwargs = dict(
        query=query,
        key=key,
        value=value,
...
def _native_attention_backward_op(
    ...
    **kwargs,
    ...
    with torch.enable_grad():
        ...
        if ctx.enable_gqa and not _SDPA_SUPPORTS_ENABLE_GQA:
            raise ValueError("`enable_gqa` is not supported by this PyTorch scaled_dot_product_attention build.")
        sdpa_kwargs = dict(
            query=query_t,
            key=key_t,
        ...
        if _SDPA_SUPPORTS_ENABLE_GQA:
            sdpa_kwargs["enable_gqa"] = ctx.enable_gqa
        out = torch.nn.functional.scaled_dot_product_attention(**sdpa_kwargs)
        out = out.permute(0, 2, 1, 3)
