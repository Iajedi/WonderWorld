...
class Flux2SingleTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # concatenated
        if single_skip_blocks is None or index_block not in single_skip_blocks: # Stable Flow modification: skip if in skip list
            if encoder_hidden_states is not None:
...
class Flux2TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if mm_skip_blocks is None or index_block not in mm_skip_blocks: # Stable Flow modification: skip if in skip list
            joint_attention_kwargs = joint_attention_kwargs or {}
...
class Flux2Transformer2DModel(
    ...
    def forward(
        ...
        # For Stable Flow-style model editing
        step_index: int = None,
        mm_copy_blocks=None,
        single_copy_blocks=None,
        mm_skip_blocks=None,
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                encoder_hidden_states, hidden_states = block(
                    ...
                    step_index=step_index,
                    index_block=index_block,
                    mm_copy_blocks=mm_copy_blocks, # Copy for double stream MMDiT
                    mm_skip_blocks=mm_skip_blocks, # Skip for double stream MMDiT
                )
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = block(
                    ...
                    step_index=step_index,
                    index_block=index_block,
                    single_copy_blocks=single_copy_blocks, # Copy for single stream DiT
                    single_skip_blocks=single_skip_blocks, # Skip for single stream DiT
                )
